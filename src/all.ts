 ...
 */
export function all<T>(error: Error, matcher: Guard<T>): T[]

/**
 * Returns every error in the tree that is an instance of the given constructor.
 */
export function all<T extends Error>(error: Error, constructor: ErrorConstructor<T>): T[]

export function all(error: Error, matcher: Guard<unknown> | ErrorConstructor<Error>): unknown[] {
